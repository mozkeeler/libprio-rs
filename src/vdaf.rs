...
pub trait Vdaf {
    ...
    /// A verification parameter, used by an Aggregator in the Verify process to ensure that the
    ...
    type AggregateShare: Aggregable;
    ...
    /// The number of rounds of communication among the Aggregators that are required before they
    /// have recovered output shares.
    fn rounds(&self) -> usize;

    /// The number of input shares generated by a Client.
...
pub trait Aggregator: Vdaf {
    /// Initial state of the Aggregator during the Verify process.
    type VerifyInit: VerifyStep<Self::OutputShare>;
    ...
    ) -> Result<Self::VerifyInit, VdafError>;
...
/// Intermediate state of an Aggregator during the Verify process.
pub trait VerifyStep<O>: Sized {
    ...
    /// messages.
    fn step<M: IntoIterator<Item = Self::Input>>(self, inputs: M) -> VerifyTransition<Self, O>;

    /// Compute the Aggregator's first message.
    fn start(self) -> Result<(Self, Self::Output), VdafError> {
        match self.step(std::iter::empty()) {
            VerifyTransition::Continue(new_state, output) => Ok((new_state, output)),
            VerifyTransition::Fail(err) => Err(err),
            VerifyTransition::Finish(_) => Err(VdafError::Uncategorized(
    ...
    fn finish<M: IntoIterator<Item = Self::Input>>(self, inputs: M) -> Result<O, VdafError> {
        match self.step(inputs) {
            VerifyTransition::Continue(_, _) => Err(VdafError::Uncategorized(
                "finish() resulted in Continue transition".to_string(),
            )),
            VerifyTransition::Fail(err) => Err(err),
            VerifyTransition::Finish(output_share) => Ok(output_share),
        }
    }
}

/// A state transition of an Aggregator during the Verify process.
pub enum VerifyTransition<S: VerifyStep<O>, O> {
    /// Continue processing.
    Continue(S, S::Output),
