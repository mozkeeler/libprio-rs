...
use crate::vdaf::{
    Aggregable, Aggregator, Client, Collector, Share, Vdaf, VdafError, VerifyStep, VerifyTransition,
};
...
impl<I: Idpf<2, 2>> Vdaf for Hits<I> {
    ...
    type Aggregate = BTreeMap<IdpfInput, u64>;
    type AggregateParam = BTreeSet<IdpfInput>;
    type PublicParam = ();
...
impl<I: Idpf<2, 2>> Aggregator for Hits<I> {
    type VerifyInit = HitsVerifyStep<I::Field>;
...
/// The state of each Aggregator during the Verify process.
...
impl<F: FieldElement> VerifyStep<Vec<F>> for HitsVerifyStep<F> {
    type Input = Vec<F>;
    type Output = Vec<F>;

    fn step<M: IntoIterator<Item = Vec<F>>>(mut self, inputs: M) -> VerifyTransition<Self, Vec<F>> {
        match self.state {
            SketchState::Ready => {
                let inputs: Vec<_> = inputs.into_iter().collect();
                if inputs.len() != 0 {
                    return VerifyTransition::Fail(VdafError::Uncategorized(format!(
                ...
                VerifyTransition::Continue(self, z_share)
            }
            ...
            SketchState::RoundOne => {
                ...
                for z_share in inputs.iter() {
                    if z_share.len() != 3 {
                        // XXX Tim- How do I print the state here?
                        return VerifyTransition::Fail(VdafError::Uncategorized(format!(
                ...
                VerifyTransition::Continue(self, y_share)
            }
            ...
            SketchState::RoundTwo => {
                ...
                VerifyTransition::Finish(self.output_share)
            }
        }
    }
}
...
impl<F: FieldElement> Aggregable for Vec<F> {
...
mod tests {
    ...
    fn test_hits() {
        ...
        assert_eq!(hits.shares(), 2);
        assert_eq!(hits.rounds(), 2);
    ...
    fn eval_vdaf<I: Idpf<2, 2>>(
        ...
        for (verify_param, input_share) in verify_params.iter().zip(input_shares.iter()) {
            let state = hits.verify(verify_param, agg_param, nonce, input_share)?;
            state0.push(state);
        }
